...
export default function VisitorCounter() {
  ...
  useEffect(() => {
    let isMounted = true;
    const badgeUrl = "https://api.visitorbadge.io/api/visitors?path=https%3A%2F%2Fchoi2156.github.io%2Faeika-dashboard%2F&label=%EB%88%84%EC%A0%81%20%EB%B0%A9%EB%AC%B8%EC%88%98&labelColor=%232b2e4a&countColor=%23e84545&style=flat-square";
    ...
    const fetchHits = async () => {
      try {
        ...
        console.warn("방문수 통계 로딩 지연/실패 (안전 모드로 가동):", err.message);
        if (isMounted) {
  ...
  return (
    <span className={`header__visitor-btn ${loading ? 'loading' : 'loaded'}`} title="대시보드 누적 방문수">
      <Eye size={14} className="visitor-btn__icon" />
      <span className="visitor-btn__label">누적 방문수</span>

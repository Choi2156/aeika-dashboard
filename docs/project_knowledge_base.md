...
- **버전 기간 연장 보정 (`end_date` 이격 및 동적 락 바이패스)**: 이전 버전의 `end_date`가 명시된 경우 기본적으로 `end_date + 1일`로 차기 시작일을 밀어내어 보호합니다. 단, 최신 기준 버전에 명시적 힌트(`cycle_override` 또는 `target_date`)가 등록된 경우에는 낡은 마감일에 구속되지 않고 힌트 주기를 100% 관철하는 **동적 락 바이패스**가 작동합니다.
- **하드코딩 요일 보정 완전 폐기**: 특정 요일(목요일/수요일)로 인위적으로 하루를 당기거나 밀던 하드코딩 보정 로직을 전면 폐기하고, `gamesConfig.js`의 `allowedWeekdays` 및 `schedule_hints.json`을 통한 데이터 주도형 자연 예측으로 전환했습니다.
- **자동 마감일 클리핑 (Auto End-Date Sanitizer)**: 차기 버전 시작일이 확정되면, 이전 버전의 `end_date`가 겹치지 않도록 차기 시작일 전날 23:59로 렌더링 단계에서 안전 클램핑합니다.
...
|---|---|---|---|
| **v1.2.6** | 2026-09-19 | **일정 예측 엔진 고도화 & 검증기(Validator) 빌드 게이트 신설**: 하드코딩 요일 보정 완전 폐기, 최신 기준 버전 힌트 스코프 오참조 수정(C-1), 방송 도약 시 고유 주기 계승 오류 해결(C-2), `target_date` 주기 지원(H-2), `while` 무한 루프 가드 탑재(H-1), 날짜 역전/중복 패치 치명적 ERROR 승격(C-3), 힌트 `note` 필드 의존성 제거, CI/CD 자동 빌드 차단 파이프라인 구축 | `scheduler.js`, `validator.js`, `validateSchedule.js`, `gamesConfig.js`, `package.json` |
| **v1.2.5** | 2026-09-08 | 이달의 종합 캘린더 다운로드 모달 추가 (2048x2048 고화질 PNG 다운로드 및 지표 급상승 견인) | `CalendarModal.jsx`, `Header.jsx` |
| **v1.2.4** | 2026-09-06 | 모바일 브라우저 환경 설정 로컬스토리지 저장 지원, 화이트 모드 시인성 전면 개편, OG 미리보기 카드 이미지 정상화 | `base.css`, `App.jsx`, `useScheduleData.js` |
| **v1.2.3** | 2026-08-21 | 추천 쇼츠 풀 게임별 10개 확장 및 스마트 셔플 도입, 공지사항 알림 시스템(상단 롤링 배너, 통합 공지 위젯, 팝업) 신설 | `NoticeModal.jsx`, `Header.jsx`, `recommended_videos.json` |
...
| **v1.0.0** | 2026-05-20 | 서브컬처 게임 일정 대시보드 최초 런칭 (원신, 스타레일, ZZZ, 명조, 엔드필드 타임라인) | 전체 시스템 |
